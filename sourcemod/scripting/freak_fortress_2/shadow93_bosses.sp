...
// Handles
Handle cooldownHUD, DelayElixir[MAXPLAYERS+1];
// Handle statusHUD;
...
public Plugin myinfo = {
	name = "Freak Fortress 2: Koishi's Boss Abilities (Forked)",
...
public void FF2_OnAbility(int boss, const char[] plugin_name, const char[] ability_name, int slot, int status)
{
	if(FF2_GetRoundState()==1 && livebosses)
	{
		...
		{
			int Rage = FF2_GetAbilityArgument(boss,THIS_PLUGIN_NAME,ability_name, "mode"); // mode
...
public void JRB_Invoke(int client)
{
	...
	{
		case 1: EmitSoundToAll(JACK_BULLET, client), TF2_AddCondition(client, TFCond_UberBulletResist, FF2_GetAbilityArgumentFloat(boss,THIS_PLUGIN_NAME,JACK_RANDOM_BUFF,"bullet_resistance_duration",10.0)), TF2_AddCondition(client, TFCond_BulletImmune, FF2_GetAbilityArgumentFloat(boss,THIS_PLUGIN_NAME,JACK_RANDOM_BUFF,"bullet_resistance_duration",10.0));
		case 2:	EmitSoundToAll(JACK_BLAST, client), TF2_AddCondition(client, TFCond_UberBlastResist, FF2_GetAbilityArgumentFloat(boss,THIS_PLUGIN_NAME,JACK_RANDOM_BUFF,"blast_resistance_duration",10.0)), TF2_AddCondition(client, TFCond_BlastImmune, FF2_GetAbilityArgumentFloat(boss,THIS_PLUGIN_NAME,JACK_RANDOM_BUFF,"blast_resistance_duration",10.0));
		case 3: EmitSoundToAll(JACK_FIRE, client), TF2_AddCondition(client, TFCond_UberFireResist, FF2_GetAbilityArgumentFloat(boss,THIS_PLUGIN_NAME,JACK_RANDOM_BUFF,"fire_resistance_duration",10.0)), TF2_AddCondition(client, TFCond_FireImmune, FF2_GetAbilityArgumentFloat(boss,THIS_PLUGIN_NAME,JACK_RANDOM_BUFF,"fire_resistance_duration",10.0));
		case 4: EmitSoundToAll(JACK_FIRE, client), TF2_AddCondition(client, TFCond_Ubercharged, FF2_GetAbilityArgumentFloat(boss,THIS_PLUGIN_NAME,JACK_RANDOM_BUFF,"uber duration",10.0));
	}
}
...
public void FHB_Invoke(int client)
{
	...
	float babifylength=FF2_GetAbilityArgumentFloat(boss, THIS_PLUGIN_NAME, FEMHEAVY_BABIFY, "babify_length");
...
void Boss_Abilities(const char[] ability_name, int boss, int rType, int bClient)
{
    switch(rType)
    {
        ...
        {
            float duration = FF2_GetAbilityArgumentFloat(boss,THIS_PLUGIN_NAME,ability_name,"duration",5.0);
            TF2_AddCondition(bClient, TFCond_UberBulletResist, duration);
            TF2_AddCondition(bClient, TFCond_BulletImmune, duration);
            TF2_AddCondition(bClient, TFCond_UberBlastResist, duration);
            TF2_AddCondition(bClient, TFCond_BlastImmune, duration);
            TF2_AddCondition(bClient, TFCond_UberFireResist, duration);
            TF2_AddCondition(bClient, TFCond_FireImmune, duration);
            int clonespawn=FF2_GetAbilityArgument(boss, THIS_PLUGIN_NAME, ability_name, "clone_count", 0); // Spawn Clone? Ability Becomes Instant Dimensional Rift
            if(clonespawn != 0)
            {
                if(!disableclone)
...
void Multiplier_Rage(const char[] ability_name, int boss, int rType)
{
    ...
    TFTeam ownerTeam = TF2_GetClientTeam(bClient);
    ...
    for (int i=0; i<minions; i++)
    {
        ...
        if(clone  != -1)
        {
            ...
            TF2_ChangeClientTeam(clone,ownerTeam);
            ...
            while((wearables=FindEntityByClassname(wearables, "tf_wearable"))!=-1)
                if((owner=GetEntPropEnt(wearables, Prop_Send, "m_hOwnerEntity"))<=MaxClients && owner>0 && TF2_GetClientTeam(owner)==ownerTeam)
            ...
            switch(rType)
            {
                case 1: // Reimu
                {
                    ...
                    spawnhealth=FF2_GetAbilityArgument(boss, THIS_PLUGIN_NAME, ability_name, "clone_health", 0);
                    int cloneprotect=FF2_GetAbilityArgument(boss, THIS_PLUGIN_NAME, ability_name, "clone_protection_time", 0); // Protect Clone at spawn?
                    if(cloneprotect!=0)
                    {
...
public Action FF2_OnLoseLife(int boss, int& lives, int maxLives)
{
	...
	int rType = FF2_GetAbilityArgument(boss,THIS_PLUGIN_NAME,BOSSRAGE, "mode");
	...
	else
	{
		if (rType==8) // Eirin's Hourai Elixir's Life Renegeration Ability (Can be fitted to Mokou, Kaguya and Chang'e since they also have consumed the Hourai Elixir)
		{
			...
			timeleft[boss]=FF2_GetAbilityArgument(boss, THIS_PLUGIN_NAME, BOSSRAGE, "init_cooldown", 60)+addtime[boss];
			addtime[boss]+=FF2_GetAbilityArgument(boss, THIS_PLUGIN_NAME, BOSSRAGE, "additional_cooldown", 60);
			...
			char charname[256], text[5120];

			for(int i = 1; i <= MaxClients; i++ )
			if(IsValidClient(i) && IsPlayerAlive(i) && TF2_GetClientTeam(i)!=TF2_GetClientTeam(client))
            {
                SetGlobalTransTarget(i);
                FF2_GetBossName(boss,charname,256,i);
...
public Action BlockDamage(int client, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	if(bClient != -1)
	{
		...
		if(TF2_GetClientTeam(client) != FF2_GetBossTeam())
		{
			...
			FF2_GetBossName(bClient,spcl,64,attacker);
...
void Teleport_Me(int client)
{
	...
	if(IsValidEdict(ii) && IsValidClient(ii) && IsPlayerAlive(ii) && TF2_GetClientTeam(ii)!=TF2_GetClientTeam(client))
	{
		AlivePlayers = true;
		break;
	}
...
public Action OnPlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(IsValidMinion(client) && !(event.GetInt("death_flags") & TF_DEATHFLAG_DEADRINGER))
	{
		...
		SummonerIndex[client] = -1;
		TF2_ChangeClientTeam(client, FF2_GetBossTeam()==TFTeam_Blue ? TFTeam_Red : TFTeam_Blue);
	}

	if(b0ss != -1 && FF2_HasAbility(b0ss, THIS_PLUGIN_NAME, BOSSRAGE))
	{
		switch(FF2_GetAbilityArgument(b0ss,THIS_PLUGIN_NAME,BOSSRAGE, "mode"))
		{
			case 3: // miku
			{
			...
			case 1, 4: // Reimu & Handsome Jack
			{
				for(int clone=1; clone<=MaxClients; clone++)
				{
					if(SummonerIndex[clone]==boss && IsValidClient(clone) && IsValidMinion(clone) && IsPlayerAlive(clone))
					{
						...
						TF2_ChangeClientTeam(clone, FF2_GetBossTeam()==TFTeam_Blue ? TFTeam_Red : TFTeam_Blue);
					}
				}
			}
		}
...
public Action OnUberDeploy(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(selfhealer!=-1)
	{
		...
		{
			if (FF2_GetAbilityArgument(selfhealer,THIS_PLUGIN_NAME,BOSSRAGE, "mode") == 8)
			{
