...
#define THIS_PLUGIN_NAME    "ff2_phatrages"
...
public void FF2_OnAbility(int index, const String:plugin_name[], const String:ability_name[], int slot, int status)
{
...
{
	new Boss=GetClientOfUserId(FF2_GetBossUserId(index));
	new damage=FF2_GetAbilityArgument(index,THIS_PLUGIN_NAME,ability_name, "damage");	        //damage
	new range=FF2_GetAbilityArgument(index,THIS_PLUGIN_NAME,ability_name, "damage radius");	        //damage radius
	new String:s[512];
	FF2_GetAbilityArgumentString(index,THIS_PLUGIN_NAME,ability_name,"sound path",s,512); //sound path
...
{
	new Boss=GetClientOfUserId(FF2_GetBossUserId(index));
	new sound=FF2_GetAbilityArgument(index,THIS_PLUGIN_NAME,ability_name, "sound");	        //sound
	new rageDamage=FF2_GetAbilityArgument(index,THIS_PLUGIN_NAME,ability_name, "damage");	        //damage
...
{
	new Boss=GetClientOfUserId(FF2_GetBossUserId(index));
	new distance=FF2_GetAbilityArgument(index,THIS_PLUGIN_NAME,ability_name, "blast speed seconds");	        //blast speed seconds
	new range=FF2_GetAbilityArgument(index,THIS_PLUGIN_NAME,ability_name, "damage radius");	        //damage radius
	new damage=FF2_GetAbilityArgument(index,THIS_PLUGIN_NAME,ability_name, "damage");	        //damage
	new aim=FF2_GetAbilityArgument(index,THIS_PLUGIN_NAME,ability_name, "aim");	        	//aim
...
stock env_shake(Float:Origin[3], Float:Amplitude, Float:Radius, Float:Duration, Float:Frequency)
{
	...
	if(DispatchSpawn(Ent))
	{
		...
		//Delete:
		RemoveEntity_Delay(Ent, 30.0);
	}
}

stock RemoveEntity_Delay(entity, Float:time = 0.0)
{
	if (time == 0.0)
	{
