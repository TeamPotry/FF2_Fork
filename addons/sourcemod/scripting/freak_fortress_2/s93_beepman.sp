...
#define THIS_PLUGIN_NAME	"s93_beepman"
...
public void FF2_OnAbility(int boss, const char[] plugin_name, const char[] ability_name, int slot, int status)
{
	...
	if(!strcmp(ability_name, SCRAMBLE)) // Keyboard scramble
	{
		...
		float dist2=FF2_GetAbilityArgumentFloat(boss, THIS_PLUGIN_NAME,ability_name, "range", 600.0);
		...
		for(int targetIdx=1;targetIdx<=MaxClients;targetIdx++)
		{
			if(IsValidLivingPlayer(targetIdx))
			{
				...
				if(dist<dist2 &&TF2_GetClientTeam(targetIdx)!=TF2_GetClientTeam(bossIdx))
				{
					scrambleKeys[targetIdx]=true;
					UnscrambleAt[targetIdx]=GetEngineTime()+FF2_GetAbilityArgumentFloat(boss, THIS_PLUGIN_NAME, ability_name, "duration");
					enemies++;
				}
			}
		}
...
public void FF2_OnCalledQueue(FF2HudQueue hudQueue, int client)
{
	...
	if(StrEqual(text, "Boss") && FF2_HasAbility(FF2_GetBossIndex(client), THIS_PLUGIN_NAME, SCRAMBLE))
	{
		if(IsOnCoolDown[client])
			Format(text, sizeof(text), "%t", "Hijack Timeleft", RoundFloat(GetEngineTime() - CooldownEndsIn[client]));
		else
...
stock void ReadCenterText(int bossIdx, const char[] ability_name, const char[] arg, char[] centerText)
{
	FF2_GetAbilityArgumentString(bossIdx, THIS_PLUGIN_NAME, ability_name, arg, centerText, 256);
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	if(bossIdx>=0 && bHopEnabled[client] && simplebHop[client] && GetEntityFlags(client) & FL_ONGROUND && buttons & IN_JUMP)
	{
		...
		if(negativefloat)
		{
			fVelocity[1] -= FF2_GetAbilityArgumentFloat(bossIdx, THIS_PLUGIN_NAME, BHOP, "velocity", 1.0);
		}
		else
		{
			fVelocity[1] += FF2_GetAbilityArgumentFloat(bossIdx, THIS_PLUGIN_NAME, BHOP, "velocity", 1.0);
		...
		if(FF2_GetAbilityArgument(bossIdx,THIS_PLUGIN_NAME,BHOP,"debug"))
		{
	...
	{
		int buttonmode=FF2_GetAbilityArgument(bossIdx, THIS_PLUGIN_NAME, HIJACK, "buttonmode"); // Use RELOAD, or SPECIAL to activate ability
		if(buttonmode==2 &&(buttons & IN_ATTACK3) || buttonmode==1 && (buttons & IN_RELOAD))
		{
			...
			float duration=FF2_GetAbilityArgumentFloat(bossIdx, THIS_PLUGIN_NAME, HIJACK, "grace period", 4.0); // Grace period between disabling and fully hijacking sentry
			...
			while((building=FindEntityByClassname(building, "obj_sentrygun"))!=-1) // Let's look for sentries to hijack
			{
				...
				if(GetVectorDistance(bossPosition, buildingPosition)<=FF2_GetAbilityArgumentFloat(bossIdx, THIS_PLUGIN_NAME, HIJACK, "range") && GetEntProp(building, Prop_Send, "m_iTeamNum")!=GetClientTeam(client))
				{
			...
			if(sentry) // Let's not drain RAGE if no sentries are within range.
			{
				...
				CooldownEndsIn[client]=GetEngineTime()+FF2_GetAbilityArgumentFloat(bossIdx, THIS_PLUGIN_NAME, HIJACK, "cooldown", 10.0);
				switch(buttonmode)
				{
					case 1: buttons &= ~IN_RELOAD;
					case 2: buttons &= ~IN_ATTACK3;
...
public Action Timer_Hijack(Handle timer, any buildingIdx) // Grace period ends here
{
	...
	if(FF2_GetRoundState()==1 && building>MaxClients && IsValidEntity(building))
	{
		if ((owner = GetEntDataEnt2(building, FindSendPropInfo("CObjectSentrygun", "m_hBuilder"))) != -1)
		{
			...
			float duration2=FF2_GetAbilityArgumentFloat(FF2_GetBossIndex(currentBossIdx), THIS_PLUGIN_NAME, HIJACK, "duration");
			if(duration2)
			{
